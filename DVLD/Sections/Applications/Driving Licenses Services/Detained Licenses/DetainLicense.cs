...
namespace DVLD.Sections.Applications.Driving_Licenses_Services.Licenses
{
    public partial class DetainLicense : Form
    {
        ...
        private void BTN_Next_Click(object sender, EventArgs e)
        {
            if (LB_DetainID.Text != "??")
            {
                TC_DetainLicense.SelectedTab = TP_DetainInfo;
                return;
            }
            ...
            if (UC_LicenseSelector.UC_LicenseInfo.LB_LicenseID.Text == "----")
                ...
                MessageBox.Show($"The current License cannot be detained, It is expired", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            else if (clsDetainedLicenses_BLL.IsDetained(Convert.ToInt32(UC_LicenseSelector.UC_LicenseInfo.LB_LicenseID.Text)))
                MessageBox.Show($"The current License already detained", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            else
                TC_DetainLicense.SelectedIndex = 1;
        }

        private void TC_LicenseApplication_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (TC_DetainLicense.SelectedTab.Text == "Detain Info")
            {
                if (LB_DetainID.Text != "??")
                {
                    TC_DetainLicense.SelectedTab = TP_DetainInfo;
                ...
                if (UC_LicenseSelector.UC_LicenseInfo.LB_LicenseID.Text == "----")
                {
                    TC_DetainLicense.SelectedTab = TP_LicenseInfo;
                    MessageBox.Show("Please select a Local License first", "No License Selected", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
                else if (Convert.ToDateTime(UC_LicenseSelector.UC_LicenseInfo.LB_ExpirationDate.Text) < DateTime.Now)
                {
